...
class Toast {
    ...
    static func toast(message: String, controller: UIViewController) {
        show(message: message, controller: controller, color: UIColor(named: "gradientBottom") ?? .black)
    }
    
    static func show(message: String, controller: UIViewController , color : UIColor = UIColor.black.withAlphaComponent(0.8)) {
        ...
        let c3 = NSLayoutConstraint(item: toastContainer, attribute: .bottom, relatedBy: .equal, toItem: controller.view, attribute: .bottom, multiplier: 1, constant: -125)
        controller.view.addConstraints([c1, c2, c3])
        }
